"""
DecentralFund DAO - Comprehensive Database Models
Complete SQLAlchemy models for all entities in the decentralized fund ecosystem
"""

from sqlalchemy import Column, Integer, String, Float, Boolean, DateTime, Text, ForeignKey, JSON, Enum as SQLEnum, Index
...
class Transaction(Base):
    ...
    transaction_metadata = Column(JSON)  # Additional transaction-specific data
...
class Notification(Base):
    ...
    # Metadata
    notification_metadata = Column(JSON)  # Additional notification-specific data
...
def create_indexes():
    ...
    indexes = [
        ...
        Index('idx_votes_proposal_id', Vote.proposal_id),
        Index('idx_transactions_user_id', Transaction.user_id),
        Index('idx_market_data_symbol', MarketData.symbol),
        Index('idx_portfolio_performance_date', PortfolioPerformance.date),
        Index('idx_sip_payments_date', SIPPayment.payment_date),
        Index('idx_proposals_status', Proposal.status),
        Index('idx_notifications_user_id', Notification.user_id, Notification.is_read)
    ]
    return indexes
